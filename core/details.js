  ...
  async function openProyecto(proyecto, options){
    ...
    function equipmentRows(equipos,tickets){
      ...
      return equipos.map(e=>{
        ...
        const equipoCodes=[];
        if(Number(e.es_critico_periodo||0)===1) equipoCodes.push('CRITICO_PERIODO');
        if(Number(e.es_critico_anio||0)===1) equipoCodes.push('CRITICO_ANIO');
    ...
    async function load(year){
      ...
      const equipmentKpis='<div class="mg-project-kpis cols-4">'+kpi('Equipos activos',metrics.equipos_activos,'🛠️','green','Portafolio activo')+kpi('Equipos detenidos',metrics.equipos_detenidos,'⛔','red','No funcionando')+kpi('Equipos críticos',metrics.equipos_criticos_anio,'💥','amber','Año actual')+kpi('Críticos del período',metrics.equipos_criticos_periodo,'‼️','indigo',criticalPeriodSubtitle)+'</div>';
