(function(){
  const MODULE_VERSION = '20260921-proyectos-metricas-anuales-v006';
  const state = {
    loaded:false,
    filtersLoaded:false,
    rows:[],
  ...
  async function refresh(){
    ...
    if(body)body.innerHTML='<tr><td colspan="17" class="proy-empty">Cargando proyectos...</td></tr>';
    try{
      ...
      if(body)body.innerHTML='<tr><td colspan="17" class="proy-empty">'+esc(e.message)+'</td></tr>';
    }
  }
  ...
  function renderTable(){
    ...
    if(!rows.length){body.innerHTML='<tr><td colspan="17" class="proy-empty">Sin proyectos para los filtros seleccionados</td></tr>';return;}
    body.innerHTML=rows.map(function(r){
      ...
      return '<tr>'+ 
        ...
        '<td class="num">'+int(r.llamadas_total_anio)+'</td>'+ 
        '<td class="date">'+date(r.ultima_llamada)+'</td>'+ 
