(function(){
  ...
  function render_uni(){renderKpis_uni();renderCharts_uni();renderCritical_uni();renderTickets_uni();const f=state_uni.data.selected_filters||{};const generated=state_uni.data.generated_at||Date.now();$('ccx-updated').textContent='Actualizado '+(window.ManttoHumanTime&&typeof window.ManttoHumanTime.mexicoCityTime==='function'?window.ManttoHumanTime.mexicoCityTime(generated):new Intl.DateTimeFormat('es-MX',{timeZone:'America/Mexico_City',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).format(new Date(generated)));const z=$('ccx-zona'),current=z.value;z.innerHTML='<option value="">Todas</option>'+((state_uni.data.filters&&state_uni.data.filters.zonas)||[]).map(x=>`<option value="${esc_uni(x)}">${esc_uni(x)}</option>`).join('');z.value=f.zona||current||'';}
  ...
  function setMonth_uni(){const today=window.ManttoHumanTime&&typeof window.ManttoHumanTime.mexicoCityDate==='function'?window.ManttoHumanTime.mexicoCityDate():new Intl.DateTimeFormat('en-CA',{timeZone:'America/Mexico_City',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());$('ccx-from').value=today.slice(0,7)+'-01';$('ccx-to').value=today;$('ccx-zona').value='';}
