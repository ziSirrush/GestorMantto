...
const state={page:1,pageSize:25,total:0,totalPages:1,rows:[],kpis:null,kpiFilterKey:null,catalogs:{tipo_cliente:[],estatus_cliente:[],estado:[],iniciales:[]}};
const controllers={list:null,kpis:null};
...
function nextController(kind){if(controllers[kind])controllers[kind].abort();const controller=new AbortController();controllers[kind]=controller;return controller;}
function abortDataRequests(){Object.keys(controllers).forEach(kind=>{if(controllers[kind])controllers[kind].abort();controllers[kind]=null;});}
async function request(path,options={}){
  const cfg=Object.assign({},options);
  const method=String(cfg.method||'GET').toUpperCase();
  const cacheTtlMs=Math.max(0,Number(cfg.cacheTtlMs||0));
  const cacheKey=cfg.cacheKey;
  delete cfg.cacheTtlMs;delete cfg.cacheKey;
  if(method==='GET'&&window.ManttoHttp&&typeof window.ManttoHttp.get==='function')return window.ManttoHttp.get(path,Object.assign({},cfg,{cacheTtlMs,cacheKey}));
  const response=await fetch(API+path,Object.assign({cache:cacheTtlMs>0?'default':'no-store',headers:headers()},cfg));
  const text=await response.text();let json={};try{json=text?JSON.parse(text):{};}catch(error){throw new Error('El backend respondió contenido no JSON.');}if(!response.ok||json.ok===false)throw new Error(json.message||json.error||('Error HTTP '+response.status));return json;
}
...
async function loadCatalogs(){try{const path='/api/ventas/clientes/catalogos';const json=await request(path,{cacheTtlMs:CATALOG_CACHE_MS,cacheKey:'catalog:'+path});state.catalogs=json.catalogos||{};fillSelect('#vcl-filter-type',state.catalogs.tipo_cliente,'Todos');fillSelect('#vcl-filter-status',state.catalogs.estatus_cliente,'Todos');fillSelect('#vcl-filter-state',state.catalogs.estado,'Todos');fillSelect('#vcl-filter-advisor',state.catalogs.iniciales,'Todos');const wrap=$('#vcl-filter-advisor-wrap');if(wrap)wrap.hidden=!Array.isArray(state.catalogs.iniciales)||state.catalogs.iniciales.length<=1;}catch(error){setStatus('Error de conexión con la API: '+error.message,'error');toast('No se pudieron cargar los filtros reales.',true);}}
async function loadKpis(options={}){const force=Boolean(options.force);const key=filterKey();if(!force&&state.kpis&&state.kpiFilterKey===key){renderKpis();return true;}const controller=nextController('kpis');try{const suffix=key?'?'+key:'';const json=await request('/api/ventas/clientes/kpis'+suffix,{signal:controller.signal});if(controllers.kpis!==controller)return false;state.kpis=json.kpis||{};state.kpiFilterKey=key;renderKpis();return true;}catch(error){if(isAbortError(error))return false;state.kpis=null;state.kpiFilterKey=null;setStatus('Error de conexión con la API: '+error.message,'error');renderKpis();return false;}finally{if(controllers.kpis===controller)controllers.kpis=null;}}
...
async function loadList(options={}){const body=$('#vcl-list-body');if(!body)return false;const silent=Boolean(options.silent);if(!silent)body.innerHTML='<tr><td colspan="8" class="vcl-loader">Cargando clientes desde Aiven...</td></tr>';const controller=nextController('list');try{const json=await request('/api/ventas/clientes?'+query(true),{signal:controller.signal});if(controllers.list!==controller)return false;state.rows=Array.isArray(json.data)?json.data:[];state.page=Number(json.pagination?.page||1);state.total=Number(json.pagination?.total||state.rows.length);state.totalPages=Number(json.pagination?.total_pages||1);setStatus('Aiven conectado · '+fmt(state.total)+' registros');body.innerHTML=state.rows.length?state.rows.map(rowHtml).join(''):'<tr><td colspan="8" class="vcl-empty">No hay clientes para los criterios seleccionados.</td></tr>';bindRows();renderPagination();return true;}catch(error){if(isAbortError(error))return false;state.rows=[];state.total=0;state.totalPages=1;setStatus('Error de conexión con la API: '+error.message,'error');if(!silent)body.innerHTML='<tr><td colspan="8" class="vcl-empty">No fue posible consultar Clientes. '+esc(error.message)+'</td></tr>';bindRows();renderPagination();return false;}finally{if(controllers.list===controller)controllers.list=null;}}
...
function renderPagination(){const el=$('#vcl-pagination');if(!el)return;const start=state.total?((state.page-1)*state.pageSize)+1:0;const end=Math.min(state.page*state.pageSize,state.total);el.innerHTML='<span>'+fmt(start)+'–'+fmt(end)+' de '+fmt(state.total)+'</span><div class="pages"><button id="vcl-prev" '+(state.page<=1?'disabled':'')+'>←</button><button class="active">Página '+state.page+' de '+Math.max(1,state.totalPages)+'</button><button id="vcl-next" '+(state.page>=state.totalPages?'disabled':'')+'>→</button></div>';$('#vcl-prev').onclick=()=>{if(state.page>1){state.page--;loadList();}};$('#vcl-next').onclick=()=>{if(state.page<state.totalPages){state.page++;loadList();}};}
async function loadAll(options={}){await Promise.all([loadKpis({force:Boolean(options.forceKpis)}),loadList({silent:Boolean(options.silent)})]);return true;}
function bind(){let timer;$('#vcl-refresh').onclick=()=>loadAll({forceKpis:true});$('#vcl-new').onclick=()=>window.ManttoRouter?.go?.('ventas-clientes-nuevo');$('#vcl-search').oninput=()=>{clearTimeout(timer);timer=setTimeout(()=>{state.page=1;loadAll();},350);};['#vcl-filter-type','#vcl-filter-status','#vcl-filter-state','#vcl-filter-advisor'].forEach(selector=>$(selector).onchange=()=>{state.page=1;loadAll();});$('#vcl-clear').onclick=()=>{$('#vcl-search').value='';['#vcl-filter-type','#vcl-filter-status','#vcl-filter-state','#vcl-filter-advisor'].forEach(selector=>$(selector).value='');state.page=1;loadAll();};}
async function mount(force=false){const view=$('#view-ventas-clientes');if(!view)return false;if(force){abortDataRequests();view.dataset.ready='0';state.kpiFilterKey=null;}if(view.dataset.ready!=='1'){const response=await fetch('./modules/ventas-clientes/ventas-clientes.html?v=20260729-fase3-v003',{cache:'default'});if(!response.ok)throw new Error('No se pudo cargar el módulo Clientes.');view.innerHTML=await response.text();view.dataset.ready='1';bind();await loadCatalogs();await loadAll({forceKpis:force});}return true;}
document.addEventListener('mantto:navigation',event=>{if(String(event?.detail?.route||'')!=='ventas-clientes')abortDataRequests();});
window.ManttoVentasClientes={init:(payload)=>mount(Boolean(payload?.createdClientId)),reload:()=>loadAll({forceKpis:true})};
})();
