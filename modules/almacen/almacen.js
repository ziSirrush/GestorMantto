(function(){
  ...
  const ROUTES = Object.freeze({
    ...
    'almacen-stock': { shortTitle:'Stock', icon:'📈', description:'Existencia actual y parámetros de stock suministrados por el Excel activo. No se inventan fórmulas faltantes.' },
    'almacen-prestamos': { shortTitle:'Préstamos', icon:'🔄', description:'Seguimiento de artículos en préstamo cuando el Excel activo contiene un conjunto compatible.' },
    'almacen-resguardos': { shortTitle:'Resguardos', icon:'🔒', description:'Consulta de resguardos cuando el Excel activo contiene un conjunto compatible.' },
    'almacen-auditoria': { shortTitle:'Auditoría', icon:'🔍', description:'Contraste físico contra el inventario activo de Aiven. F4 V002 no guarda histórico.' }
  });
  ...
  const state = {
    ...
    audit:{ view:'select', company:'todas', warehouseQuery:'', selectedCompany:'', selectedWarehouse:'', catalogs:null, session:null, result:null },
    requestSeq:0
  };
  ...
  async function get(path, options){
    if(window.ManttoHttp && typeof window.ManttoHttp.get === 'function') return window.ManttoHttp.get(path, options || {});
  ...
  function shell(route, body){
    ...
    return '<div class="alm-shell">' +
      ...
      statusPill(state.source ? 'Excel activo en Aiven' : 'Sin carga activa', state.source ? 'ok' : 'warn') + '</section>' + body + '</div>';
  }
  ...
  function sourceCard(){
    if(!state.source) return '<section class="alm-card alm-source-card"><div><strong>Fuente temporal Excel</strong><span>No existe un lote activo en Aiven.</span></div>' + statusPill('Sin carga','warn') + '</section>';
    const invRows=state.source.datasets&&state.source.datasets.INVENTARIO?state.source.datasets.INVENTARIO.filas:state.source.filas; return '<section class="alm-card alm-source-card"><div><strong>Fuente temporal Excel</strong><span>' + escapeHtml(state.source.archivoOrigen || 'Archivo') + ' · ' + escapeHtml(state.source.hojaOrigen || 'Hoja') + ' · ' + number(invRows,0) + ' filas de inventario</span><small>Corte: ' + dateText(state.source.fechaCorte) + ' · Importado: ' + dateText(state.source.fechaImportacion) + '</small></div>' + statusPill('Lote activo','ok') + '</section>';
  }
  ...
  function renderDashboardLoaded(data){
    ...
    return shell('almacen-dashboard',
      ...
      '</section>' +
      '<section class="alm-card"><div class="alm-section-head"><div><h2>Resumen por empresa</h2><p>Derivado del lote Excel activo.</p></div></div><div class="alm-company-summary-grid">' + (companyRows || '<span>Sin empresas.</span>') + '</div></section>' +
      ...
      '<section class="alm-card"><div class="alm-ranking-head"><div><h2>Rankings de artículos</h2><p>Información actual del lote activo. “Más movidos” sigue pendiente de historial.</p></div></div><div class="alm-grid-2 alm-ranking-grid"><section class="alm-ranking-panel"><div class="alm-ranking-title"><span>📦</span><div><strong>Top 15 por volumen</strong><small>Cantidad física</small></div></div>' + rankingRows(data.topByVolume, function(v){return number(v,2);}) + '</section><section class="alm-ranking-panel"><div class="alm-ranking-title"><span>💰</span><div><strong>Top 15 por valor</strong><small>Valor estimado</small></div></div>' + rankingRows(data.topByValue, money) + '</section></div><div class="alm-movements-pending"><div><strong>🔄 Top 15 más movidos</strong><span>No se calcula con snapshots de Excel; requiere movimientos históricos/BG.</span></div>' + statusPill('Pendiente BG','warn') + '</div></section>'
    );
  }

  async function loadDashboard(view){
    view.innerHTML = shell('almacen-dashboard', loading('Consultando inventario activo...'));
    try{
  ...
  function inventoryMain(data){
    ...
    return inventoryFilters(data) + '<section class="alm-card alm-table-card"><div class="alm-section-head"><div><h2>Inventario consolidado</h2><p>Lote Excel activo almacenado en Aiven.</p></div></div>' + renderTable(['Artículo','Categoría','Empresa','Físico','P. Unit.','Valor','Alm.'],rows,900) + inventoryPager(data && data.pagination,'inventory') + '</section>';
  }
  ...
  async function loadInventario(view){
    ...
    try{
      ...
      if(!state.source){view.innerHTML=renderInventoryShell(empty('No existe un lote activo. Si tienes autorización, carga el Excel desde el bloque superior.'));bindInventory(view);return;}
  ...
  function operationalDatasetCard(type,label){
    ...
    if(!dataset)return '<section class="alm-card alm-source-card"><div><strong>'+escapeHtml(label)+'</strong><span>El lote activo no contiene un conjunto '+escapeHtml(type)+' compatible.</span><small>Inventario puede seguir funcionando normalmente. Carga un .xlsx con los encabezados requeridos para habilitar esta vista.</small></div>'+statusPill('Sin datos compatibles','warn')+'</section>';
    return '<section class="alm-card alm-source-card"><div><strong>'+escapeHtml(label)+'</strong><span>'+escapeHtml(dataset.hojaOrigen||'Hoja')+' · '+number(dataset.filas,0)+' filas</span><small>Archivo: '+escapeHtml(state.source.archivoOrigen||'—')+' · Corte: '+dateText(state.source.fechaCorte)+'</small></div>'+statusPill('Datos activos','ok')+'</section>';
  }
  ...
  function renderStock(data){
    ...
    return shell('almacen-stock',
      ...
      '<section class="alm-kpi-grid">'+
        kpi('Artículos analizados','📊',number(k.articulos,0),'Inventario activo','blue')+
  ...
  async function loadStock(view){
    view.innerHTML=shell('almacen-stock',loading('Consultando stock activo...'));
    try{
  ...
  function renderLoanSummary(summary){
    ...
    return loanAgeCards(summary)+'<section class="alm-card alm-table-card"><div class="alm-section-head"><div><h2>Préstamos por responsable</h2><p>Concentrado del conjunto PRESTAMO activo.</p></div>'+statusPill('Fuente Excel','ok')+'</div>'+renderTable(['Responsable','Artículos','Cantidad','Valor total','% del total','Días en préstamo','Desde','Sitios','Detalle'],rows,1060)+'</section>';
  }
  ...
  function renderLoanDetail(detail){
    ...
    return '<section class="alm-card alm-filter-card"><div class="alm-filter-row alm-filter-row-main"><label class="alm-field"><span>Responsable</span><select id="alm-loan-responsible">'+responsibleOptions+'</select></label><label class="alm-field"><span>Antigüedad</span><select id="alm-loan-age"><option value="todas">Todas</option>'+['1-6 MESES','6-15 MESES','MAYOR A 15 MESES'].map(function(value){return '<option value="'+value+'"'+(state.loans.age===value?' selected':'')+'>'+value+'</option>';}).join('')+'</select></label><label class="alm-field"><span>Buscar</span><input id="alm-loan-query" type="search" value="'+escapeHtml(state.loans.query)+'" placeholder="Artículo, sitio o AG..."></label></div></section>'+
      ...
      '<section class="alm-card alm-table-card"><div class="alm-section-head"><div><h2>Detalle de artículos en préstamo</h2><p>Desglose del conjunto PRESTAMO activo.</p></div>'+statusPill('30 por página','ok')+'</div>'+renderTable(['Artículo','AG','Responsable','Sitio','Cant.','Costo','Fecha','Días','Antigüedad'],rows,980)+operationalPager('loans',detail.pagination)+'</section>';
  }

  function renderLoans(){
    const catalogs=state.loans.catalogs||{};
    if(!state.source)return shell('almacen-prestamos',empty('No existe un lote activo de Almacén.'));
  ...
  function renderGuards(){
    ...
    if(!state.source)return shell('almacen-resguardos',empty('No existe un lote activo de Almacén.'));
  ...
  function auditModeCard(){
    return '<section class="alm-card alm-audit-mode"><div><strong>Modo de esta fase: contraste temporal</strong><span>El esperado se consulta del lote INVENTARIO activo en Aiven. Los conteos capturados viven únicamente en esta sesión y no se escriben en la tabla de importación.</span></div>'+statusPill('Sin persistencia','warn')+'</section>';
  }
  ...
  function renderAuditSelect(){
    const catalogs=state.audit.catalogs||{warehouses:[]};
    if(!catalogs.source)return shell('almacen-auditoria',sourceCard()+auditModeCard()+empty('No existe un lote activo de inventario en Aiven.'));
    if(!catalogs.available)return shell('almacen-auditoria',sourceCard()+auditModeCard()+empty('El lote activo no contiene almacenes con existencia positiva para auditar.'));
    ...
    return shell('almacen-auditoria',sourceCard()+auditModeCard()+'<section class="alm-card"><div class="alm-section-head"><div><h2>Nueva auditoría</h2><p>Selecciona un almacén del lote activo. La muestra se genera exclusivamente con artículos INVENTARIO existentes en Aiven.</p></div>'+statusPill('5% · 70/30','ok')+'</div><div class="alm-audit-method"><div><b>1</b><span><strong>Almacén</strong><small>Empresa + almacén con físico mayor a cero.</small></span></div><div><b>2</b><span><strong>Muestra</strong><small>5% de referencias; 70% desde el grupo de mayor valor y 30% aleatorio.</small></span></div><div><b>3</b><span><strong>Contraste</strong><small>Captura encontrado y revisa diferencias contra el valor esperado del lote.</small></span></div></div></section><section class="alm-card alm-filter-card"><div class="alm-filter-row alm-filter-row-main"><label class="alm-field"><span>Empresa</span><select id="alm-audit-company">'+companyOptions+'</select></label><label class="alm-field alm-field-grow"><span>Buscar almacén</span><input id="alm-audit-warehouse-query" type="search" value="'+escapeHtml(state.audit.warehouseQuery)+'" placeholder="Nombre de almacén..."></label></div></section><section class="alm-card"><div class="alm-section-head"><div><h2>Almacenes disponibles</h2><p>Fuente: INVENTARIO del lote activo.</p></div></div>'+auditWarehouseCards()+'<div class="alm-audit-generate"><div>'+(canGenerate?'<strong>'+escapeHtml(state.audit.selectedWarehouse)+'</strong><small>'+escapeHtml(state.audit.selectedCompany)+'</small>':'<strong>Sin selección</strong><small>Selecciona un almacén para continuar.</small>')+'</div><button id="alm-audit-generate" type="button"'+(canGenerate?'':' disabled')+'>Generar muestreo</button></div></section>');
  }
  ...
  function renderAuditCapture(){
    ...
    const rows=(session.items||[]).map(function(item,index){
      ...
      return '<article class="alm-audit-item is-'+tone+'"><div class="alm-audit-item-main"><span class="alm-audit-index">'+String(index+1).padStart(2,'0')+'</span><div><strong>'+escapeHtml(item.article||item.code||'Sin descripción')+'</strong><small>'+escapeHtml(item.code||'Sin código')+(item.category?' · '+escapeHtml(item.category):'')+'</small></div></div><div class="alm-audit-expected"><span>Esperado</span><strong>'+number(item.expected,2)+'</strong><small>'+money(item.expectedValue)+'</small></div><div class="alm-audit-capture"><label><span>Encontrado</span><input type="number" min="0" step="any" data-audit-found="'+index+'" value="'+(has?escapeHtml(item.found):'')+'" placeholder="0"></label><button type="button" data-audit-ok="'+index+'">✓ Correcto</button></div><div class="alm-audit-difference"><span>Diferencia</span><strong>'+(diff===null?'—':(diff>0?'+':'')+number(diff,2))+'</strong><small>'+(has&&item.unitValue!==null&&item.unitValue!==undefined?money(auditFoundValue(item)-Number(item.expectedValue||0)):'Valor —')+'</small></div></article>';
    }).join('');
    const allDone=metrics.completed===metrics.total&&metrics.total>0;
    return shell('almacen-auditoria',sourceCard()+auditModeCard()+'<section class="alm-card alm-audit-session-head"><button type="button" class="alm-link-button" id="alm-audit-cancel">← Cancelar</button><div><h2>Auditoría en curso · '+escapeHtml(session.warehouse)+'</h2><p>'+escapeHtml(session.company)+' · '+number(session.sampleSize,0)+' de '+number(session.totalReferences,0)+' referencias · Sesión '+escapeHtml(String(session.sessionId||'').slice(0,8))+'</p></div>'+statusPill(number(metrics.completed,0)+' / '+number(metrics.total,0),'ok')+'</section><section class="alm-kpi-grid">'+kpi('Capturados','🧾',number(metrics.completed,0)+' / '+number(metrics.total,0),'Conteos físicos','blue')+kpi('Coincidencias','✅',number(metrics.exact,0),'Misma cantidad esperada','green')+kpi('Esperado','📦',number(metrics.expectedPieces,2)+' pz','Muestra seleccionada','purple')+kpi('Diferencia parcial','±',(metrics.pieceDifference>0?'+':'')+number(metrics.pieceDifference,2)+' pz','Con capturas actuales','amber')+'</section><section class="alm-audit-capture-list">'+rows+'</section><section class="alm-card alm-audit-footer"><div><strong>'+ (allDone?'Conteo completo':'Faltan '+number(metrics.total-metrics.completed,0)+' artículos') +'</strong><small>'+ (allDone?'Ya puedes cerrar el contraste. No se guardará en Aiven.':'Captura una cantidad o usa “Correcto” en cada artículo.') +'</small></div><button id="alm-audit-finish" type="button"'+(allDone?'':' disabled')+'>Finalizar contraste</button></section>');
  }

  function finalizeAudit(){
    if(!state.audit.session)return;
    const metrics=auditMetrics(state.audit.session);
    if(metrics.completed!==metrics.total){alert('Completa todos los artículos antes de finalizar.');return;}
    state.audit.result={session:state.audit.session,metrics:metrics,finishedAt:new Date().toISOString()};
    state.audit.view='result';
  }
  ...
  function renderAuditResult(){
    ...
    const rows=session.items.map(function(item){
      ...
      const valueDiff=item.unitValue===null||item.unitValue===undefined?null:found*Number(item.unitValue)-Number(item.expectedValue||0);
    ...
    return shell('almacen-auditoria',sourceCard()+auditModeCard()+'<section class="alm-card alm-audit-result-head"><div><span class="alm-eyebrow">Resultado de sesión</span><h2>'+escapeHtml(session.warehouse)+'</h2><p>'+escapeHtml(session.company)+' · '+escapeHtml(new Date(result.finishedAt).toLocaleString('es-MX'))+'</p></div><div class="alm-audit-result-actions"><button type="button" class="alm-link-button" id="alm-audit-new">Nueva auditoría</button><button type="button" id="alm-audit-print">Imprimir / Guardar PDF</button></div></section><section class="alm-kpi-grid">'+kpi('Renglones correctos','✅',number(metrics.exact,0)+' / '+number(metrics.total,0),number(metrics.matchPercent,1)+'% coincidencia','green')+kpi('Piezas esperadas','📦',number(metrics.expectedPieces,2),'Muestra auditada','blue')+kpi('Piezas encontradas','🔎',number(metrics.foundPieces,2),(metrics.pieceDifference>0?'+':'')+number(metrics.pieceDifference,2)+' diferencia','amber')+kpi('Impacto valor','💰',money(metrics.valueDifference),metrics.valued+' artículos con valor comparable','purple')+'</section><section class="alm-card alm-table-card"><div class="alm-section-head"><div><h2>Detalle del contraste</h2><p>Resultado calculado en memoria. No constituye histórico persistido.</p></div>'+statusPill('Solo sesión','warn')+'</div>'+renderTable(['Artículo','Código','Esperado','Encontrado','Diferencia','Valor esperado','Dif. valor','Estado'],rows,980)+'</section>');
  }
  ...
  async function loadAudit(view){
    view.innerHTML=shell('almacen-auditoria',loading('Consultando inventario activo para Auditoría...'));
    try{
      if(state.audit.view==='select'||!state.audit.catalogs){
        state.audit.catalogs=await get('/api/almacen/auditoria/catalogos',{force:true});
  ...
  async function generateAuditSample(view){
    ...
    view.innerHTML=shell('almacen-auditoria',loading('Generando muestra desde el lote activo...'));
    try{
      const data=await get('/api/almacen/auditoria/muestra'+qs({company:state.audit.selectedCompany,warehouse:state.audit.selectedWarehouse}),{force:true});
      if(!data.sample)throw new Error('No se recibió una muestra de auditoría.');
      state.audit.session=data.sample;
      state.audit.session.items=(data.sample.items||[]).map(function(item){return Object.assign({},item,{found:null});});
      state.audit.result=null; state.audit.view='capture';
  ...
  function bindAudit(view){
    ...
    view.querySelectorAll('[data-audit-found]').forEach(function(input){input.addEventListener('input',function(){const idx=Number(input.dataset.auditFound);if(!state.audit.session||!state.audit.session.items[idx])return;state.audit.session.items[idx].found=input.value===''?null:Number(input.value);});input.addEventListener('change',function(){view.innerHTML=renderAuditCapture();bindAudit(view);});});
    view.querySelectorAll('[data-audit-ok]').forEach(function(button){button.addEventListener('click',function(){const idx=Number(button.dataset.auditOk);if(!state.audit.session||!state.audit.session.items[idx])return;state.audit.session.items[idx].found=Number(state.audit.session.items[idx].expected||0);view.innerHTML=renderAuditCapture();bindAudit(view);});});
    const cancel=view.querySelector('#alm-audit-cancel');if(cancel)cancel.addEventListener('click',function(){if(!confirm('¿Cancelar este contraste? Los conteos de esta sesión se perderán.'))return;state.audit.session=null;state.audit.result=null;state.audit.view='select';loadAudit(view);});
    const finish=view.querySelector('#alm-audit-finish');if(finish)finish.addEventListener('click',function(){finalizeAudit();view.innerHTML=renderAuditResult();bindAudit(view);});
  ...
  function init(route){
    ...
    view.dataset.almacenReady='5-v002';
