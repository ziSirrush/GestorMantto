...
const state={page:1,pageSize:25,rows:[],totalPages:0,catalogs:{asesores:[],administrativos:[],zonas:[],anios_cierre:[]},initialized:false,lastDataSignature:''};
...
function fillCatalogs(){const now=currentYearCdmx();const years=[...new Set((state.catalogs.anios_cierre||[]).map(Number).filter(Boolean))].sort((a,b)=>b-a);if(!years.includes(now))years.unshift(now);$('#vv-filter-year').innerHTML='<option value="todos">Todos</option>'+years.map(y=>'<option value="'+y+'">'+y+'</option>').join('');$('#vv-filter-year').value=String(now);$('#vv-filter-advisor').innerHTML='<option value="">Todos</option>'+(state.catalogs.asesores||[]).map(personOption).join('');$('#vv-filter-admin').innerHTML='<option value="">Todos</option>'+(state.catalogs.administrativos||[]).map(personOption).join('');$('#vv-filter-zone').innerHTML='<option value="">Todas</option>'+(state.catalogs.zonas||[]).map(z=>'<option value="'+esc(z.zona??z)+'">'+esc(z.zona??z)+'</option>').join('');const advisors=state.catalogs.asesores||[];const admins=state.catalogs.administrativos||[];$('#vv-filter-advisor-wrap').hidden=advisors.length<=1;$('#vv-filter-admin-wrap').hidden=admins.length<=1;}
function query(){const q=new URLSearchParams({page:String(state.page),page_size:String(state.pageSize)});const search=$('#vv-search').value.trim();const year=$('#vv-filter-year').value;const advisor=$('#vv-filter-advisor').value;const admin=$('#vv-filter-admin').value;const zone=$('#vv-filter-zone').value;if(search)q.set('search',search);if(year)q.set('anio',year);if(advisor)q.set('id_asesor',advisor);if(admin)q.set('id_admin',admin);if(zone)q.set('zona',zone);return q.toString();}
...
function bind(){let timer;$('#vv-search').addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(()=>{state.page=1;load();},350);});['vv-filter-year','vv-filter-advisor','vv-filter-admin','vv-filter-zone'].forEach(id=>$('#'+id)?.addEventListener('change',()=>{state.page=1;load();}));$('#vv-clear').addEventListener('click',()=>{$('#vv-search').value='';$('#vv-filter-year').value=String(currentYearCdmx());$('#vv-filter-advisor').value='';$('#vv-filter-admin').value='';$('#vv-filter-zone').value='';state.page=1;load();});$('#vv-refresh').addEventListener('click',load);$('#vv-body').addEventListener('click',e=>{const target=e.target.closest('[data-view], tr[data-row-index]');if(!target)return;const rowIndex=target.dataset.rowIndex??target.closest('tr')?.dataset.rowIndex;openDetail(target.dataset.view,rowIndex);});$('#vv-pagination').addEventListener('click',e=>{const b=e.target.closest('[data-page]');if(!b)return;state.page=Number(b.dataset.page)||1;load();});}
