...
function date(v){if(!v)return'—';const raw=String(v).trim();const m=raw.match(/^(\d{4})-(\d{2})-(\d{2})(?:$|[T\s])/);if(m)return m[3]+'/'+m[2]+'/'+m[1];const d=new Date(raw);return Number.isNaN(d.getTime())?raw.slice(0,10):new Intl.DateTimeFormat('es-MX',{timeZone:'America/Mexico_City',day:'2-digit',month:'2-digit',year:'numeric'}).format(d);}
function humanDateTime(v){return window.ManttoHumanTime&&typeof window.ManttoHumanTime.formatDateTime==='function'?window.ManttoHumanTime.formatDateTime(v,{fallback:String(v||'—')}):String(v||'—');}
