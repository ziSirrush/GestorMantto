...
function openProject(p){if(window.ManttoDetails&&typeof window.ManttoDetails.openProyecto==='function')window.ManttoDetails.openProyecto(p.id||p.proyecto,{template:'cliente-unificado',source:'ventas-fotos-mapa',projectName:p.proyecto,cliente:p.cliente||''});}
...
function render(){
  ...
  box.querySelectorAll('[data-project-id]').forEach(el=>{
    ...
    el.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();openProject(p);}});
  });
}
async function load(){const [e,f]=await Promise.all([getJson('/api/ins-fl?limit=5000'),getJson('/api/ins-fl/proyectos/fotografias?limit=5000').catch(()=>({data:[]}))]);proyectos=build(Array.isArray(e.data)?e.data:[]);const photoMap=new Map((Array.isArray(f.data)?f.data:[]).map(x=>[String(x['ID Proyecto']||'').trim(),x]));proyectos.forEach(p=>Object.assign(p,photoMap.get(p.id)||{}));populate();render();const s=document.getElementById('vfm-aiven-status');if(s)s.innerHTML='<span class="py-connection-dot"></span><span>Aiven conectado · '+proyectos.filter(p=>fotos(p).length).length+' con foto</span>';}
async function mount(force){const view=document.getElementById('view-ventas-fotos-mapa');if(!view)return false;if(force)view.dataset.ready='0';if(view.dataset.ready!=='1'){const r=await fetch('./modules/ventas-fotos-mapa/ventas-fotos-mapa.html?v=20260725-v001',{cache:'no-store'});if(!r.ok)throw new Error('No se pudo cargar Fotos Mapa.');view.innerHTML=await r.text();view.dataset.ready='1';document.getElementById('vfm-buscar')?.addEventListener('input',render);document.getElementById('vfm-estado')?.addEventListener('change',render);await load();}return true;}
