...
const state={page:1,pageSize:25,rows:[],totalPages:0,catalogs:{asesores:[],anios_perdidos:[],razones_perdido:[]},visibility:{acceso_total:false},initialized:false};
...
async function init(){const view=$('#view-ventas-perdidos');if(!view)return;if(!view.dataset.loaded){const r=await fetch('./modules/ventas-perdidos/ventas-perdidos.html',{cache:'no-store'});if(!r.ok)throw new Error('No se pudo cargar la vista Perdidos.');view.innerHTML=await r.text();view.dataset.loaded='1';bind();await loadCatalogs();}await load();state.initialized=true;}
