(function(){
  ...
  const state={rows:[],movements:[],cut:null,cutError:null,loaded:false};
  ...
  <section class="dl-grid dl-two">
    ...
    <article class="dl-card dl-section"><div class="dl-section-head"><div><h2>Movimientos semanales</h2><p id="dl-mov-subtitle">Último corte autónomo cerrado.</p></div><small id="dl-mov-count">—</small></div><div class="dl-table-wrap"><table class="dl-table"><thead><tr><th>Tipo</th><th>PP NS</th><th>Proyecto</th><th>Estatus</th></tr></thead><tbody id="dl-mov-body"></tbody></table></div></article>
  </section>
  ...
  function render(){
    const rows=state.rows,sin=rows.filter(isSin),mov=state.movements;
  ...
  async function load(){setStatus('','Cargando Aiven...');try{const results=await Promise.all([fetchRows(),fetchLatestCut().then(data=>({data,error:null})).catch(error=>({data:null,error}))]);state.rows=results[0];state.cut=results[1].data;state.cutError=results[1].error;state.movements=state.cut&&Array.isArray(state.cut.movimientos_json)?state.cut.movimientos_json:[];window.ManttoLogisticaStore={rows:state.rows,loadedAt:Date.now()};state.loaded=true;render();setStatus('ok','Aiven conectado · '+state.rows.length+' registros');}catch(e){setStatus('error',e.message);$('dl-pipeline').innerHTML='<div class="dl-empty">No se pudo cargar Dashboard Logística.</div>';}}
  function init(){
