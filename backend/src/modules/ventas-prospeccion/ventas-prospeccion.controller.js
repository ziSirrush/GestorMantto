...
const quotationRelationService = require('./ventas-prospeccion-cotizacion.service');
...
async function getFileAccess(req, res, next) {
  try { return res.status(200).json(await service.getFileAccess(req.params.id, req.params.idArchivo, req.query || {}, buildActionContext(req))); }
  catch (error) { return sendKnownError(error, res, next); }
}

async function deleteFile(req, res, next) {
  try { return res.status(200).json(await service.deleteFile(req.params.id, req.params.idArchivo, buildActionContext(req))); }
  catch (error) { return sendKnownError(error, res, next); }
}
...
async function searchSources(req, res, next) {
  try { return res.status(200).json(await service.searchSources(req.query || {}, buildActionContext(req))); }
  catch (error) { return sendKnownError(error, res, next); }
}

async function getCaptureCatalogs(req, res, next) {
  try { return res.status(200).json(await service.getCaptureCatalogs(buildActionContext(req))); }
  catch (error) { return sendKnownError(error, res, next); }
}

async function getClientContacts(req, res, next) {
  try { return res.status(200).json(await service.getClientContacts(req.query || {}, buildActionContext(req))); }
  catch (error) { return sendKnownError(error, res, next); }
}

async function createVisit(req, res, next) {
  try { return res.status(201).json(await service.createVisit(req.body || {}, req.files || [], buildActionContext(req))); }
  catch (error) { return sendKnownError(error, res, next); }
}

async function getDetailCatalogs(req, res, next) {
  try { return res.status(200).json(await service.getDetailCatalogs(buildActionContext(req))); }
  catch (error) { return sendKnownError(error, res, next); }
}
...
async function updateProspectionStatus(req, res, next) {
  try {
    ...
    // Fase 2: una visita cuyo origen fue NUEVO no puede saltar directamente a
    // Cotizado sin una cotización relacionada. La relación se resuelve por el
    // endpoint dedicado y conserva el historial existente de Prospección.
    await quotationRelationService.assertStatusTransitionAllowed(
      req.params.id,
      req.body?.estatus,
      actionContext
    );
...
async function linkQuotationAndSetQuoted(req, res, next) {
  try {
    const actionContext = buildActionContext(req);
    const statusBefore = await ventasNotifications.captureProspeccionStatus_gnral(req.params.id);
    const result = await quotationRelationService.linkQuotationAndSetQuoted(
      req.params.id,
      req.body || {},
      actionContext
    );
    const notificationResult = result?.estatus_actualizado === true
      ? await ventasNotifications.notifyProspeccionStatus_gnral(req.params.id, statusBefore, actionContext)
      : { created: 0 };
