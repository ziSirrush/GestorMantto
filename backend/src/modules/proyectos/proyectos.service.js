  ...
  try {
    ...
    const [equipos] = await db.query(`
      SELECT ${portafolioBaseSelect},
        ...
        (SELECT MAX(tblt.fecha_reporte) FROM tickets tblt WHERE tblt.codigo_equipo = p.numero_equipo AND tblt.fecha_reporte >= MAKEDATE(YEAR(${sqlMexicoCityToday()}), 1) AND tblt.fecha_reporte < DATE_ADD(${sqlMexicoCityToday()}, INTERVAL 1 DAY) AND UPPER(COALESCE(tblt.responsabilidad,'')) LIKE '%BLT%') AS ultimo_blt,
