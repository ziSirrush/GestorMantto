...
const ATRAPADOS_KEYWORDS_EXP=Object.freeze(['atrapado','atrapada','encerrado','encerrada','persona atrapada','personas atrapadas','rescate']);
const AGUA_KEYWORDS_EXP=Object.freeze(['agua','inundacion','inundación','filtracion','filtración','gotera','humedad','anegado']);
const VOLTAJE_KEYWORDS_EXP=Object.freeze(['voltaje','variacion de voltaje','variación de voltaje','sobre voltaje','bajo voltaje','pico de voltaje','falla electrica','falla eléctrica','corte de luz','falla de energia','falla de energía','sin energia','sin energía','apagon','apagón']);

function positiveInt_exp(value,fallback,min,max){const p=Number.parseInt(value,10);return Number.isNaN(p)?fallback:Math.max(min,Math.min(max,p));}
function normalizeFilter_exp(value){return String(value||'').trim().slice(0,150);}
function normalizeText_exp(value){return String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim().toLowerCase();}
function normalizeTicketStatus_exp(value){const n=normalizeText_exp(value);if(n.includes('cerr'))return'Cerrado';if(n.includes('curso')||n.includes('proceso'))return'En curso';if(n.includes('abier')||n.includes('pend'))return'Abierto';return String(value||'').trim()||'Abierto';}
function normalizeResponsibility_exp(value){const n=normalizeText_exp(value);if(n.includes('blt'))return'BLT';if(n.includes('client'))return'CLIENTE';return String(value||'').trim().toUpperCase();}
function dateKeyInTimeZone_exp(value){const f=new Intl.DateTimeFormat('en-CA',{timeZone:TIME_ZONE_EXP,year:'numeric',month:'2-digit',day:'2-digit'});const r={};for(const p of f.formatToParts(value||new Date()))if(p.type!=='literal')r[p.type]=p.value;return`${r.year}-${r.month}-${r.day}`;}
function validDateKey_exp(value){const t=String(value||'').trim();return/^\d{4}-\d{2}-\d{2}$/.test(t)?t:'';}
function textBlob_exp(row){return[row.descripcion,row.causa,row.accion_en_cierre].filter(Boolean).join(' ').toLowerCase();}
function containsAny_exp(row,keywords){const blob=textBlob_exp(row);return keywords.some(k=>blob.includes(k));}
function finiteNumber_exp(value,max){const p=Number(value);return(!Number.isFinite(p)||p<=0||p>max)?null:p;}
function dateTimeEpoch_exp(dateValue,timeValue){const date=String(dateValue||'').slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return null;let time=String(timeValue||'').trim();if(!time||time.toLowerCase()==='null')time='00:00:00';time=time.replace(/^1899-12-3[01]T/i,'').replace(/\.\d+Z?$/i,'').trim();const ap=time.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(AM|PM)$/i);let h,m,s;if(ap){h=Number(ap[1]);m=Number(ap[2]);s=Number(ap[3]||0);if(ap[4].toUpperCase()==='PM'&&h<12)h+=12;if(ap[4].toUpperCase()==='AM'&&h===12)h=0;}else{const military=time.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?/);if(!military)return null;h=Number(military[1]);m=Number(military[2]);s=Number(military[3]||0);}const p=date.split('-').map(Number);return Date.UTC(p[0],p[1]-1,p[2],h,m,s);}
function closeMinutes_exp(row){const start=dateTimeEpoch_exp(row.fecha_llegada_fecha,row.h_llegada),end=dateTimeEpoch_exp(row.fecha_cierre_fecha,row.h_solucion);if(start!==null&&end!==null){const hours=(end-start)/3600000;if(hours>0&&hours<=MAX_CLOSE_HOURS_EXP)return Math.round(hours*60);}const fallback=finiteNumber_exp(row.tiempo_solucion,MAX_CLOSE_HOURS_EXP);return fallback===null?null:Math.round(fallback*60);}
...
function receptionDateSql_exp(alias){const a=alias||'p';return`COALESCE(STR_TO_DATE(NULLIF(TRIM(${a}.fecha_recepcion_mantenimiento),''),'%Y-%m-%d'),STR_TO_DATE(NULLIF(TRIM(${a}.fecha_recepcion_mantenimiento),''),'%d/%m/%Y'),STR_TO_DATE(NULLIF(TRIM(${a}.fecha_recepcion_mantenimiento),''),'%d-%m-%Y'))`;}

async function getEntregasRecientes_exp(req){
  const months=positiveInt_exp(req.query&&(req.query.meses||req.query.meses_recepcion),DEFAULT_RECEPCION_MONTHS_EXP,1,MAX_RECEPCION_MONTHS_EXP);
  ...
  const availableDatesSql=`SELECT DATE_FORMAT(DATE(t.fecha_reporte),'%Y-%m-%d') AS fecha FROM tickets t WHERE t.codigo_equipo IN (${placeholders}) AND t.fecha_reporte IS NOT NULL GROUP BY DATE_FORMAT(DATE(t.fecha_reporte),'%Y-%m-%d') ORDER BY DATE_FORMAT(DATE(t.fecha_reporte),'%Y-%m-%d') DESC`;
  const [datesResult]=await repository.query(availableDatesSql,codes);
  const availableDates=(datesResult||[]).map(r=>String(r.fecha||'')).filter(Boolean);
