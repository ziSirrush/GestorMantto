...
async function getPreventivosSupervisor(req, res) {
  try {
    ...
    if (!/^\d{4}-\d{2}$/.test(mes)) {
      return res.status(400).json({ ok: false, message: 'El parámetro mes debe usar el formato YYYY-MM.' });
    }
    ...
    const porZona = new Map(servicios.map(row => [String(row.zona_clave || ''), {
    ...
    for (const row of supervisores) {
      ...
      const zonaClave = zona.toUpperCase().replace(/[-\s]/g, '');
      const conteo = porZona.get(zonaClave) || { programados: 0, realizados: 0 };
      item.zonas.push(zona);
      item.programados += conteo.programados;
      item.realizados += conteo.realizados;
    }

    const data = [...porSupervisor.values()].map(item => ({
      ...item,
      porcentaje: item.programados ? Math.round((item.realizados / item.programados) * 100) : 0
    }));

    return res.json({ ok: true, source: 'aiven', mes, data });
  } catch (error) {
    return res.status(500).json({
      ok: false,
